...
if (-not (Test-Path -LiteralPath $dotnet)) {
    ...
    Write-Host "Baixando o instalador oficial do .NET 8 SDK..."
...
if (-not (Test-Path -LiteralPath $classData)) {
    if (-not (Test-Path -LiteralPath $UabeaZip)) {
        throw "UABEA nao encontrado em $UabeaZip. Informe o zip com -UabeaZip."
    }
    ...
    try {
        ...
        if (-not $entry) { throw "classdata.tpk nao existe dentro de $UabeaZip." }
